...
featuredalt: ""
featuredpath: "/post/2017-09-05-young-families_files/figure-html"
linktitle: ''
type: "post"
---
...
<style>
.halves {width:48%;display:inline-block;padding:2%;align:top;}
</style>
...
As expected, population change is a strong confounding variable. For example, Surrey saw in overall increase in the number of children whereas the City of Vancouver experienced a slight drop, but the share of chidren in the total population dropped more in Surrey than it did in Vancouver. Both views address different questions.

Next we can look at where young families with children live, where we call families "young" if the oder parent is below 40 years old.
...
na_rows <- data %>% filter(is.na(`0-4`) | is.na(`5-9`) | is.na(`10-14`)) %>% as.data.frame %>% select(c("GeoUID","year",age_groups))
na_rows
all_na <- na_rows %>% pull("GeoUID")
```
Of note here is that the 2011 data has one CT with suppressed data on the number of children. There are several routes we can take moving forward, we could set the NA numbers to zero, we could exlcude the offending CT from the analysis or we could estimate the number of children from the overall City of Vancouver counts. There is no perfect answer to this, for the purpose of our analysis that separates different regions within CoV we think it's best to omit the CT.
...
Census trackts change over time, so it's always good to do a quick visual check what we got the split consistent over time, as well as remind ourselves what they are.
...
total_graph <- function(data,title,groups = age_groups, y_label="Number of Children Under 15") {
  ...
  plot_data <- data %>% 
    ...
    tidyr::gather(key = `Age Group`, value = Count, groups) 
  plot_data$`Age Group` <- as.factor(plot_data$`Age Group`)
  levels(plot_data$`Age Group`)=groups
...
share_graph <- function(data,title,groups=age_groups, y_label="Share of Population Under 15") {
  ...
  share_plot_data <- data %>% 
    ...
    summarize(`0-4`=sum(`0-4`)/sum(Population),  
              ...
              Population=sum(Population)) %>%
    tidyr::gather(key = `Age Group`, value = Count, groups) 
  share_plot_data$`Age Group` <- as.factor(share_plot_data$`Age Group`)
  levels(share_plot_data$`Age Group`)=groups
...
```{r graph, fig.height=8, fig.width=8}
...
## VSB
If we are interested in different regions, it is now easy to adapt the analysis. Let's look at Vancouver School Board, who is struggling to deal with changing enrolment patterns in their schools. The above analysis of central vs outer regions in the City of Vancouver touches on some of the problems, but overall the City of Vancouver does not match the geography of SD 39. To remedy this, let's just re-run the overview analysis using the correct region.


```{r, message=FALSE, warning=FALSE}
vsb_regions=list(CT=c("9330069.01","9330069.02"),CSD=c("5915022","5915803"))
...
```
Here we notice that the trend of declining population of 5 to 14 year olds has been reversed, although the share of children in that age bracked out of the total population still declined as the next graph shows.
