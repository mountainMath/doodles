...
## Opening up the API
Then we opened up the API. That forced us to further formalise the protocol. And we also initiated a [public R package](https://github.com/mountainMath/cancensus) that acts as glue between R, a popular platform for statistical analysis, and the CensusMapper API. This enables anyone to easily use these tools. And to contribute to their development, which people did. Open source can be a powerful tool to drive innovation and ensure high quality, and this is exactly what happend with the `cancensus` R package. Through the collaborative work of others `cancensus` development was much faster than I could have done it, and most importantly, it became a much better package than I could have done on my own.
